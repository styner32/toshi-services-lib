...
from toshi.utils import validate_signature, validate_address, parse_int
try:
    from toshi.request import generate_request_signature_data_string
    from toshi.ethereum.utils import data_decoder, ecrecover
...
TOSHI_TIMESTAMP_HEADER = "Toshi-Timestamp"
TOSHI_SIGNATURE_HEADER = "Toshi-Signature"
TOSHI_ID_ADDRESS_HEADER = "Toshi-ID-Address"

TOSHI_TIMESTAMP_QUERY_ARG = "toshiTimestamp"
TOSHI_SIGNATURE_QUERY_ARG = "toshiSignature"
TOSHI_ID_ADDRESS_QUERY_ARG = "toshiIdAddress"
    ...
    if ETHEREUM_SUPPORTED:
        def verify_request(self):
            ...
            if TOSHI_ID_ADDRESS_HEADER in self.request.headers:
                ...
                raise JSONHTTPError(400, body={'errors': [{'id': 'bad_arguments', 'message': 'Missing Toshi-ID-Address'}]})
            ...
            if TOSHI_SIGNATURE_HEADER in self.request.headers:
                ...
                raise JSONHTTPError(400, body={'errors': [{'id': 'bad_arguments', 'message': 'Missing Toshi-Signature'}]})
            ...
            if TOSHI_TIMESTAMP_HEADER in self.request.headers:
                ...
                raise JSONHTTPError(400, body={'errors': [{'id': 'bad_arguments', 'message': 'Missing Toshi-Timestamp'}]})

            timestamp = parse_int(timestamp)
            if timestamp is None:
                raise JSONHTTPError(400, body={'errors': [{'id': 'invalid_timestamp',
                                                           'message': 'Given Toshi-Timestamp is invalid'}]})

            if not validate_address(expected_address):
                raise JSONHTTPError(400, body={'errors': [{'id': 'invalid_id_address', 'message': 'Invalid Toshi-ID-Address'}]})

            if not validate_signature(signature):
                raise JSONHTTPError(400, body={'errors': [{'id': 'invalid_signature', 'message': 'Invalid Toshi-Signature'}]})
        ...
        def is_request_signed(self, raise_if_partial=True):
            ...
            count_toshi_headers = sum(1 if x in self.request.headers else 0 for x in [TOSHI_ID_ADDRESS_HEADER, TOSHI_SIGNATURE_HEADER, TOSHI_TIMESTAMP_HEADER])
