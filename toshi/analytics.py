...
def encode_id(toshi_id):
    return sha256(toshi_id.encode('utf-8')).hexdigest() if toshi_id else None

class AnalyticsMixin:

    def track(self, toshi_id, event, data=None, add_user_agent=True):
        ...
        if self.application.mixpanel_instance:
            self.application.mixpanel_instance.track(encode_id(toshi_id), event, data)

    def people_set(self, toshi_id, data=None):
        if self.application.mixpanel_instance:
            self.application.mixpanel_instance.people_set(encode_id(toshi_id), data)
